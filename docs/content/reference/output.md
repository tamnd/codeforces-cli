...
describes what users get. Pick a format with `-o`, or let cf choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
cf <command> -o table   # aligned columns for reading
cf <command> -o jsonl   # one JSON object per line, for piping
cf <command> -o json    # a single JSON array
cf <command> -o csv     # spreadsheet friendly
cf <command> -o tsv     # tab-separated
cf <command> -o url     # just the URL column
cf <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
cf <command> --fields id,title,url
```
...
```bash
cf <command>            # a table, because this is a terminal
cf <command> | wc -l    # JSONL, because this is a pipe
```
